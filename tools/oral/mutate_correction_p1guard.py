...
MUTATIONS = [
    ...
    ("E1-D", "drop subject inheritance across coordinated clauses",
     sub_in_file(CURRENTNESS, "            elif group == subject_group:",
                 "            elif False:", count=1),
    ...
    ("E4-T", "make <td> stop labelling its row (revert to <th> only)",
     sub_in_file(VISIBLE, 'CELL_TAGS = frozenset(("td", "th"))',
                 'CELL_TAGS = frozenset(("th",))', count=1),
     CONTROLS, "E4_first_table_cell_labels_its_row"),
    ("E1-P", "stop a pronoun carrying its subject to the next sentence",
     sub_in_file(CURRENTNESS,
                 "            elif group == subject_group + 1 and "
                 "PRONOUN_SUBJECT.match(sent):",
                 "            elif False:", count=1),
     CONTROLS, "E1_pronoun_carries_the_subject_one_sentence"),
    ("F-X", "take the examiner-stem exemption away from the fire-main check",
     sub_in_file(CURRENTNESS,
                 "                  | EXCUSED_CLASSES) or _stem_echo(stack):",
                 "                  ) or False:", count=1),
     CONTROLS, "firemain_excuses_examiner_wording_like_bmp5_does"),
    ("H-D", "stop treating a dated frame as historical",
     sub_in_file(CURRENTNESS,
                 r'    r"before \d{4}|until \d{4}|up to \d{4}|region[- ]locked|"',
                 '    r"(?!x)x|"', count=1),
     CONTROLS, "historical_date_framing_is_a_denial"),
    ("E1-O", "anchor the pronoun rule on the bare pronoun again",
     sub_in_file(
         CURRENTNESS,
         r'    r"^(?:%s\b,?\s+){0,2}(?:it|they|this|these|those)\b" % _OPENER, re.I)',
         r'    r"^(?:it|they|this|these|those)\b", re.I)', count=1),
     CONTROLS, "E1_adverbial_opener_does_not_hide_the_pronoun"),
    ("E1-V", "drop the past-tense verbs from the clause test",
     sub_in_file(
         VISIBLE,
         r'         r"governed|covered|specified|mandated|recommended|introduced|"',
         r'         r"(?!x)x|"', count=1),
     CONTROLS, "E1_past_tense_verbs_still_form_a_clause"),
    ("W-D", "revert the exemption to an exact attribute-value match",
     sub_in_file(
         CURRENTNESS,
         '            r\'(?:"[^"]*\\b(?:q-card|topic-block)\\b[^"]*"\'',
         '            r\'(?:"(?:q-card|topic-block)"\'', count=1),
     CONTROLS, "currentness_exemption_survives_markup_variation"),
    ("F-Q", "take the question rule away from the fire-main check",
     sub_in_file(CURRENTNESS, '            if sent.rstrip().endswith("?"):',
                 "            if False:", count=1),
     CONTROLS, "firemain_excuses_a_question_like_bmp5_does"),
