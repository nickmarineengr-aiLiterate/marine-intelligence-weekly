...
from render_common import (REPO_ROOT, esc, esc_attr, strip_tags, read_css,
                           topbar, head_meta, footer, GATE_STUB)
...
def build_year_page(specs, year, publish):
    ...
    o.extend(head_meta(strip_tags(title), strip_tags(desc),
                       '/meoclass1/pastpapers/questions-%d.html' % year, publish))
    a('<style>')
    a(read_css())
    a(YEAR_CSS)
    a('</style>')
...
def main():
    ...
    for year in years:
        ...
        st = write(path, build_year_page(specs, year, args.publish))
        print('questions-%d.html  %s' % (year, st))
