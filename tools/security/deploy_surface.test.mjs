...
// added for a test, so a small matcher below implements the gitignore
// subset this repository's .vercelignore uses -- and REFUSES any
// syntax outside that subset (negation, `**`, character classes,
// escapes). If someone writes a pattern the matcher cannot evaluate
// faithfully, the suite fails rather than guessing.
// =============================================================
...
// -------------------------------------------------------------
// Minimal gitignore matcher (subset). Last match wins; no negation
// is accepted, so any match means "ignored".
// -------------------------------------------------------------
const UNSUPPORTED = /(^!|\*\*|\[|\\)/;

function parsePatterns(text, source) {
  const out = [];
  for (const raw of text.split(/\r?\n/)) {
    const line = raw.replace(/\s+$/, "");
    if (!line || line.startsWith("#")) continue;
    if (UNSUPPORTED.test(line)) {
      throw new Error(`${source}: pattern "${line}" uses syntax this test's ` +
        `matcher does not implement (negation, **, [], or escapes). Extend ` +
        `the matcher deliberately or rewrite the pattern.`);
    }
    let pat = line;
...
/** Is `relPath` (posix, file) ignored under `patterns`? */
function isIgnored(relPath, patterns) {
  const parts = relPath.split("/");
  // candidates: every ancestor directory path, then the file itself
  const ancestors = [];
...
const vercelignoreText = fs.readFileSync(path.join(ROOT, ".vercelignore"), "utf8");
