...
## 1c. The 22 holdings-denial flags — adjudicated and closed — 2026-08-15

Not a paper review: the single largest remaining factual-correctness class, run to completion.
`holdings_layer` reported **22 flags across 6 papers and 16 questions** (QP2302, QP2304, QP2407,
QP2408, QP2410, QP2411). Every one was adjudicated against the governed record before anything
was edited.

| | |
|---|---|
| **Verdicts** | **15 questions CORRECTED** (false denial), **1 PASS on the denial itself** — QP2302-Q8 correctly says *"does not hold **in the solved corpus**"*, which is true; its coupled *"not one of the eight is usable"* was the falsehood and was corrected. **0 deferred, 0 detector false positives among the original 22.** No claim lacked evidence |
| **What was actually wrong** | Denials of sittings MIW **does** hold. Eight pointed-at sittings are intelligence-only (`QP2201`, `QP2203`, `QP2209`, `QP2210`, `QP2211`, `QP2212`, `QP2107`, `QP2108`); **seven are SOLVED** (`QP2301`, `QP2302`, `QP2303`, `QP2307`, `QP2309`, `QP2402`, `QP2404`). In eleven cases the denied stem is **character-identical** to the question denying it |
| **Root cause is STALENESS, not authoring error** | Every denial of a *solved* sitting was **true when written** and became false when a later session solved that paper. `QP2309` was solved after `QP2407`; `QP2303` after `QP2411`. A correct adjudication decays |
| **The canonical layer had already self-corrected** | `recurrence_model.py` computes families chronologically from solved transcriptions, so it **already** headed `QP2407-Q1` at `QP2309-Q1`, `QP2410-Q9` at `QP2301-Q6` and `QP2411-Q8` at `QP2303-Q6` while the prose still denied them. The three-layer separation held |
| **NOT candidate-facing** | `recurrence_adjudication` is on the exclusion list in `build_index.py` and `build_solvedqp_manifest.py`, and `specs/` is `.vercelignore`d. **Proved, not assumed:** a full publish build after the corrections changed **zero generated bytes**, and all seven corrected phrases return 0 hits outside `specs/`. This class is authoring-layer debt, not a live defect |
| **Provenance NOT rewritten** | `reuse_tier`, `reused_from` and `recurrence_class` are left exactly as authored on every question. They record the authoring decision; rewriting them to match the present family would falsify the record. Only present-tense factual claims were corrected. No `text_verbatim`, printed mark or source misprint was touched |
| **Checker refined — SENTENCE SCOPING** | A denial whose sentence **names** sitting tokens is now adjudicated against those tokens only; a denial naming none is still blanket and still checked against everything. Purely positional, no reading of meaning. Three controls, each mutation-tested to prove it is live: a denial naming a **held** sitting still fires; a denial scoped to the one token it denies stays silent while a different held token sits on the same question; a blanket denial still fires |
| **Result** | **22 → 0.** `recurrence boundary: 0 failure(s)`, self-test all guards fire when broken |
| **Invariants** | Six-year **61 papers / 549 questions (39 solved / 22 intelligence-only)** unchanged; no paper graduated, no family changed. Product **39 / 351**, 2023 11/11. Security **284/284**. Two full publish builds byte-identical; `delivery_gate --strict --verify-derivation` PASS; UI behaviour PASS 39 pages |

**Open, recorded, not fixed here — a DIFFERENT class.** The same staleness that caused this one
also leaves ~19 authoring-layer sites asserting that a paper *is* unsolved intake when it has since
been solved (`QP2411` ×7, `QP2501` ×3, `QP2410`, `QP2302`, `QP2406`, `QP2502`, `QP2509`, `QP2511`).
Three were corrected in passing because they sat inside sentences this session was already
rewriting. These live in the same non-delivered fields and are **P2**, not a launch blocker. There
is no detector for them; the sweep used here was a crude regex that does not distinguish tense.
