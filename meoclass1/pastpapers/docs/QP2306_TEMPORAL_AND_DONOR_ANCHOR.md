...
|---|---|---|---|---|---|
| **Q1** | **EXACT** | `QP2303-Q1` | March 2023 | **−3 months (earlier)** | Stems **verbatim identical**, including both limb letters. Same year, same temporal regime. **Earliest root is April 2021, not March 2023** — `QP2104-Q2` and `QP2210-Q9` are both held, transcribed and verbatim identical; six-member family. Corrected at laptop review, §8 |
...
| **Q9** | **EXACT wording ancestry, no answer donor** — *corrected at laptop review, see §8* | wording: `QP2210-Q7` (**verbatim**, unsolved) · answer family: `QP2402-Q3` / `QP2310-Q1` (hull-form optimisation) · `QP2503-Q8` / `QP2307-Q7` (hull and propeller maintenance) | Oct 2022 · 2023–2025 | **−8 months** · mixed | The three named technologies — air cavity, surface texturing, microbubbles — are **FRESH as an answer**. ~~Host hint `2022/OCT/Q7` is a paper MIW does not hold~~ — **THAT WAS WRONG.** `QP2210` is held and transcribed, and `QP2210-Q7` is verbatim identical |
...
### 4.1 TSCR-3 IS ENGAGED BY THIS PAPER — DECLINED AT AUTHORING, **CLOSED UPSTREAM BEFORE PUBLICATION**

> **LAPTOP REVIEW, 15 August 2026 — this section was correct when written and is now stale.**
> **TSCR-3 is CLOSED.** The corpus was corrected upstream on 2026-08-15 at corpus commit
> `7441cc0`; `entryIntoForce` now reads **`2022-11-01`**, verified this session by reading the
> corrected register at the corpus `origin/main`. **The date this paper uses never changed**, so no
> answer moved — but every disclosure describing the register as *still wrong* and the referral as
> *still open* was stale and has been corrected in the spec. `TSCR-4` closed in the same commit.
> The account below is retained as the authoring record.
...
|---|---|---|
| **No *quotation-ready* consolidated MARPOL Annex VI.** *Corrected at laptop review:* official-source PDFs of the **MARPOL consolidated 2022 edition** and of **Annex VI + NOx Technical Code (5th ed. 2023)** **are held** — but both are **image-only with no text layer**, so neither is citation-ready at provision level. The limitation stands in effect; the original wording ("holds no text") did not | Regulation numbering and content for regulations 20, 26, 27, 28 are established from the recitals and cross-references **inside `MEPC.346(78)`**, read directly. **No regulation is quoted verbatim** | `C_ACCEPTED_LIMITATION` (Q7) |
...
## 8. LAPTOP REVIEW — 15 August 2026

Reviewed and integrated onto `main` per `LAPTOP_REVIEW_AND_INTEGRATION_PROTOCOL.md`. Source
adjudicated independently from the printed paper; all nine answers adjudicated on substance.

**Verdicts: Q2 · Q3 · Q5 · Q6 · Q8 PASS. Q1 · Q4 · Q7 · Q9 CORRECTED.**

The producer's source work is the strongest in the batch and none of it was disturbed: the `(Ill)`
misprint is preserved and named, the Q4 stem is correctly rejoined across the page break with the
host's marketing block excluded, and the 96-against-100 marks anomaly is recorded rather than
repaired. Q7's temporal reversals were re-derived and are right — `MEPC.377(80)` appears only as an
exclusion, the SEEMP sits at regulation 26 from 1 November 2022, and the Initial 2018 Strategy is
correctly operative. Q1's CLC treatment keeps the residue-carrying tanker inside CLC, so the
`QP2302` error was not propagated. Q2 defines a **very** serious marine casualty (which
`MSC.255(84)` does define) and never mentions GISIS. Trap suite: **314 checks, 0 failures**; no ISM
"regulation" defect in any form.

### 8.1 Four corrections, in one class and one perishable claim

**Three of the four are the same defect: authored prose asserting MIW does not hold something it
holds.** The governed intelligence layer and the corpus were both right; the prose was wrong.

| Q | Claim as authored | Truth | Sites |
|---|---|---|---|
| **Q9** | host hint `2022/OCT/Q7` points at "a paper MIW **DOES NOT HOLD AND HAS NEVER TRANSCRIBED**" | **`QP2210` is held and transcribed.** `QP2210-Q7` (October 2022) is **verbatim identical** to this stem. The governed classifier independently returns **`EXACT_REPEAT`**, `first_seen` October 2022 | 4 + `recurrence_class` |
| **Q1** | "**only** the March 2023 entry corresponds to a question MIW has transcribed" | **Three do.** `QP2104-Q2` (April 2021) and `QP2210-Q9` (October 2022) are both held, transcribed and **verbatim identical**. The classifier returns a **six-member family** rooted at **April 2021** | 1 + `host_recurrence_hint` |
| **Q4** | "MIW holds no licensed SOLAS text" | **SOLAS 2024 consolidated edition is held** — three parts, **555 of 556 pages carry an extractable text layer**. The authoring choice (answer the equipment limb by function, not carriage requirement) is right; the reason given was false. Corrected to the real reason plus the real limitation: **the held edition post-dates the June 2023 sitting** | **7** |
| **Q7** | "TSCR-3 remains OPEN"; "MIW holds no licensed consolidated MARPOL Annex VI **text**" | **TSCR-3 and TSCR-4 both CLOSED upstream** at corpus `7441cc0`, 2026-08-15; register now reads `2022-11-01`. And the consolidated 2022 edition **is** held — but **image-only, no text layer**, so it is held-not-quotation-ready. The limitation survives; the wording did not | 5 |

**Root cause of the Q1 miss is recorded, because it is mechanical.** `host_recurrence_hint` had been
transcribed across the source's line breaks with the tokens shifted by one — `"2015"`, `"AUG 2015"`,
`"NOV 2017"`, `"OCT 2021/APR/Q2"` — so the two held ancestors were hidden inside a mangled token that
could not be resolved by eye. Re-parsed to the printed tokens.

**Root cause of the Q9 miss is a search-scope error, and it is the more general one.** Five targeted
searches were run and all five came back empty — but they were run over the **solved answer corpus**,
not over the **six-year intelligence layer**. The correct conclusion from an empty result is *"the
subject has no built answer"*, never *"the subject has no precedent"*. Both are now stated in the spec.

### 8.2 What did NOT change, and why that matters

**No answer content moved on any of the four.** All three holdings corrections and the TSCR closure
are *evidence-and-provenance* corrections: `reused_from` stays `null` on Q9 because `QP2210` is
unsolved and there is no answer to inherit, and Q7's date was right before and after. This is the
distinction the protocol draws between **wording ancestry** and **answer donor**, and QP2306 is now
the clearest example of it in the corpus — an exact printed recurrence whose answer had to be
authored from nothing.

**A perishable claim is a claim with a shelf life.** TSCR-3 was accurately open when this branch was
pushed and was closed upstream before it was reviewed. That is the second consecutive paper on which
a True Source referral went stale mid-flight, so corpus-status disclosures are now verified against
the corpus `origin/main` at review time rather than inherited from the branch.

### 8.3 Reported, not rebuilt

`recurrence_check.py` guards the **provenance boundary** — that no host sitting token reaches a
generated artefact — and passes cleanly here. It does **not** reconcile a spec's authored
`recurrence_class` against the class the governed model computes, which is why Q9 could ship as
`topical_family` while the model said `EXACT_REPEAT`. **Three papers have now carried a false
"not held" claim.** A narrow deterministic guard is warranted — assert that no spec asserts
non-holding of any paper present in the intelligence layer, and that authored `recurrence_class`
does not contradict the derived family class. It is **reported here, not built**, per invariant 7:
the defect in this paper is fixed, and the guard belongs to the session that owns the checker.
