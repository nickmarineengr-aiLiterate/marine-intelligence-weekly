...
| **`A.1118(30)`** Revised Guidelines on the implementation of the ISM Code by Administrations, adopted 6 December 2017 | Operative — verified against the Organization's own published resolution. Its successor is **`A.1188(33)`** (6 December 2023), future here. **NOT `A.1184(33)`** — see `TS-REFERRAL-QP2311-3` |
...
| **`A.1188(33)`** 2023 ISM implementation guidelines (revoking `A.1118(30)`) | 6 December 2023 | +1 month |
| `A.1184(33)` Guidelines on places of refuge for ships in need of assistance | 6 December 2023 | +1 month |
...
| **Q5** | Alternatives to tin-based antifouling paints — hull roughness by paint type; oil incorporation in foul-release coatings; personnel and health safety | **FAMILY** (solved set) · **EXACT wording recurrence** of `QP2209-Q5` | `QP2306-Q5` (June 2023) shares the preamble only; `QP2301-Q2` / `QP2404-Q5` / `QP2409-Q8` share the paint taxonomy; `QP2503-Q8` / `QP2307-Q7` supply hull-roughness management | +5 to +21 months | **The preamble recurs across the solved set; all three printed limbs are new to the SOLVED set but NOT to MIW.** `QP2209-Q5` (September 2022, intelligence-only) prints this question word for word — preamble and all three limbs — and normalises IDENTICAL under the governed marks-safe rule. The host's `2022/SEP/Q5` pointer is therefore **confirmed, not unverifiable**. It is **wording ancestry only**: QP2209 carries no answer, so `reused_from` stays **null**. Only the CDP / SPC / hybrid / foul-release taxonomy is inherited from solved papers |
...
| **Q8** | Developments in marine diesel engines, and retrofit methods enabling slow and ultra-slow steaming | **FRESH** (no answer donor) · **NEAR wording recurrence** of `QP2204-Q3` | none. Wording ancestor: `QP2204-Q3` (April 2022, intelligence-only). Family context: `QP2306-Q9` (skin friction), `QP2303-Q4` / `QP2310-Q5` (NOx measures) | −19 months (ancestor) | No solved question addresses slow steaming or de-rating retrofits, so no answer donor exists. **But `QP2204-Q3` sets substantially this task nineteen months earlier** — *"What are the engine developments & retrofitting which were adopted by engine manufacturers to enable hassle free slow steaming & ultra-slow steaming of ships?"* — sharing the distinctive printed phrase *hassle free slow steaming*. The host printed **no** hint for this question, so only the six-year layer could see it. Wording ancestry only; `reused_from` **null**. Authored fresh, anchored on the 2023 GHG Strategy and the CII pressure |
...
| `A.1185(33)` / `A.1186(33)` → **`A.1155(32)`** / **`A.1156(32)`**; `A.1188(33)` → **`A.1118(30)`** | Q1, Q6 | Citing 33rd Assembly instruments adopted one month after the sitting |
...
| **`TS-REFERRAL-QP2311-3`** | **NEW — register MISIDENTIFICATION; a factual defect, not a direction note.** `10-amendment-register/AMENDMENT_REGISTER.md` line 69 records `A.1184(33)` as *GUIDANCE (Administrations' implementation)*. **`A.1184(33)` is in fact the Guidelines on places of refuge for ships in need of assistance.** The Assembly guidance on implementation of the ISM Code by Administrations is **`A.1188(33)`**, adopted 6 December 2023, revoking **`A.1118(30)`** of 6 December 2017. All three verified against the Organization's own published resolutions. The register's number is **not consumed** by this paper | **RAISED. No corpus file was touched.** Corpus-wide exposure recorded in the session report. **2026-08-15: promoted to `TSCR-9`** in `TRUE_SOURCE_CORRECTION_REQUESTS.md` — still OPEN at corpus `7441cc0`, and the referral named only one of two surfaces: `ISM-Code/INSTRUMENT_LOG.md` line 19 carries the same wrong attribution and is the surface `QP2502`-Q3 consumed. `QP2406`-Q5 and `QP2502`-Q1/Q3 corrected to `A.1188(33)` |
