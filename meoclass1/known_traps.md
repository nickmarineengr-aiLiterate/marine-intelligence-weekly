...
### 31. QB3_A_CheatSheet.html GZ curve diagram — Angle of Loll marker was plotted at the curve's trough, not the GZ=0 crossing
In the "GZ Curve & Angle of Loll — The Three GM States" diagram, the angle-of-loll marker and label ("GZ=0, ship sits here at rest") were positioned at the negative-GM curve's most-negative point (the trough), which is not where GZ=0 — a self-contradiction, since the label claimed GZ=0 at a point clearly below the GZ=0 line. Per standard texts (Ship Stability for Masters and Mates, Fig 31.2 / Fig 9.9; Ship Stability — Notes & Examples), the angle of loll is defined as the heel angle where GZ = 0, i.e. where the curve crosses back through zero after its initial negative excursion — between 0° and the loll angle the righting lever is negative (capsizing moment, ship heels further); only beyond the loll angle does GZ become positive (righting moment back toward the loll angle, not upright). Moved the marker circle and drop-line to the curve's actual zero-crossing point on the SVG path (a guaranteed on-path Bezier endpoint). Corrected in QB3_A_CheatSheet.html, v1.0 → v1.1.
GREP: SKIP — SVG coordinate/diagram error, not a text phrase; repo-wide grep for "ship sits here at rest" confirmed this diagram is not duplicated elsewhere.
...
| 2026-08-07 | Entry 31: QB3_A_CheatSheet.html GZ curve diagram — Angle of Loll marker plotted at the trough instead of the GZ=0 crossing, contradicting standard texts | Nixon Antony (screenshot review) |
