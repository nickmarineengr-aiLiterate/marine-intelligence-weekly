...
**Commit:** 2b784ce77911bdcff37e5494784745b5a3b3221d. This entry lands in the substantive publication commit together with the content fix and the register transition, per `corrections/README.md` Lifecycle §4. That commit cannot contain its own hash, so this token is written locally and is replaced with the real hash by an immediately following metadata-only commit; both are pushed together, and the token never reaches the remote. The hash recorded here is the SUBSTANTIVE publication commit, not the metadata commit that writes it.
